...
const OVERVIEW_DATE_RANGE_OPTIONS = [
  { value: "7d", label: "Last 7 days", periodLabel: "last 7 days" },
  { value: "30d", label: "Last 30 days", periodLabel: "last 30 days" },
  { value: "90d", label: "Last 3 months", periodLabel: "last 3 months" },
  { value: "180d", label: "Last 6 months", periodLabel: "last 6 months" },
  { value: "365d", label: "Last 12 months", periodLabel: "last 12 months" },
  { value: "all", label: "All time", periodLabel: "all time" },
] as const;
...
export function UserDownloadsDashboard({ embedded = false }: { embedded?: boolean }) {
  ...
  const periodLabel =
    OVERVIEW_DATE_RANGE_OPTIONS.find((option) => option.value === dateRange)
  ...
  return (
    <div className={embedded ? "" : "min-h-screen bg-white p-6"}>
      <div className={embedded ? "" : "max-w-7xl mx-auto"}>
        {/* Header - full when standalone, compact when embedded */}
        <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 ${embedded ? "flex-row items-center justify-between" : ""}`}>
          <div>
            {!embedded && (
            ...
            <p className={`text-sm text-gray-500 ${embedded ? "mb-0" : "mt-0.5"}`}>
              {overview?.overview?.totalDownloads ?? data?.summary?.totalDownloads ?? 0} total
              {" · "}
              {overview?.overview?.downloadsInPeriod ?? 0} in {periodLabel}
              {" · "}
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {OVERVIEW_DATE_RANGE_OPTIONS.map((option) => (
