...
import { SystemMonitoringFilters } from "./SystemMonitoringFilters";
import {
  PAGE_SIZE,
  SystemMetricsTab,
  ErrorLogsTab,
...
export function SystemMonitoringDashboard() {
  ...
  const fetchSystemMetrics = useCallback(
    (page = 1) => {
      fetch(
        `/api/admin/analytics/system-metrics?${buildParams(page, { status })}`,
      )
        .then((res) => res.json())
  ...
  const refresh = useCallback(() => {
    if (data !== null) setIsRefreshing(true);
    Promise.all([
      fetchData(),
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        {/* Title for table tabs */}
        {isTableTab && (
          <h1 className="text-lg font-semibold text-zinc-900 tracking-tight mb-6">
            System Monitoring
          </h1>
        )}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <div className="mb-0 w-full">
            <TabsList className="flex h-11 w-full gap-1 rounded-lg bg-white p-1 mb-6">
