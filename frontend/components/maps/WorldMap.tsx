...
import bundledWorldTopology from "./data/countries-110m.json";
...
export const WorldMap = memo(({ applications, config = {} }: WorldMapProps) => {
  ...
  const geographySource = useMemo((): UseGeographiesProps["geography"] => {
    if (finalConfig.geoUrl?.trim()) return finalConfig.geoUrl.trim();
    return bundledWorldTopology as UseGeographiesProps["geography"];
  }, [finalConfig.geoUrl]);
  ...
  return (
    ...
    >
      ...
      >
        <ComposableMap
          className="map-composable block w-full max-w-full"
